...
echo "-- Building MongoDB 3.2 image"
docker build -t mongo-3.2 ../3.2/
docker network create mongo_net
DIR_VOLUME=$(pwd)/vol32
mkdir -p ${DIR_VOLUME}/backup

echo
echo "-- Testing backup/checking for MongoDB 3.2"
docker run --name base_1 -d --net mongo_net mongo-3.2 --storageEngine wiredTiger --smallfiles --noprealloc; sleep 20
...
echo
echo "-- Backup"
docker run -it --rm --net mongo_net -e 'MONGO_MODE=backup' -v ${DIR_VOLUME}/backup:/tmp/backup mongo-3.2 --host base_1 -u backuper -p pass; sleep 10
echo
echo "-- Clear"
...
docker run -it --rm -e 'MONGO_CHECK=default' -e 'COLLECTION_NAME=db'  -v ${DIR_VOLUME}/backup:/tmp/backup mongo-3.2  2>&1 | tail -n 1 | grep -c 'Fail'; sleep 10

echo
echo "-- Restore backup for MongoDB 3.2"
docker run --name base_1 -d -e 'MONGO_RESTORE=default' -v ${DIR_VOLUME}/backup:/tmp/backup mongo-3.2 --storageEngine wiredTiger --smallfiles --noprealloc; sleep 25
...
echo
echo "-- Testing Replica Set Cluster for MongoDB 3.2"
docker run --name node_1 -d --net mongo_net mongo-3.2 --storageEngine wiredTiger --smallfiles --noprealloc --replSet "rs_test"; sleep 20
docker run --name node_2 -d --net mongo_net mongo-3.2 --storageEngine wiredTiger --smallfiles --noprealloc --replSet "rs_test"; sleep 20
docker run --name node_3 -d --net mongo_net mongo-3.2 --storageEngine wiredTiger --smallfiles --noprealloc --replSet "rs_test"; sleep 20
echo
...
echo "-- Backup replica"
mkdir -p ${DIR_VOLUME}/backup
docker run -it --rm --net mongo_net -e 'MONGO_MODE=backup' -v ${DIR_VOLUME}/backup:/tmp/backup mongo-3.2 -h node_2 --oplog; sleep 10
echo "-- Clear"
...
echo
echo "-- Testing Sharded Cluster for MongoDB 3.2"
echo
echo "-- Create shards"
...
echo
echo "-- Testing Sharded Cluster with Replica set for MongoDB 3.2"
echo
echo "-- Create replica set #1"
docker run --name node_11 -d --net mongo_net mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs1"; sleep 20
docker run --name node_12 -d --net mongo_net mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs1"; sleep 20
docker run --name node_13 -d --net mongo_net mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs1"; sleep 20
echo
...
docker run --name node_21 -d --net mongo_net mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs2"; sleep 20
docker run --name node_22 -d --net mongo_net mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs2"; sleep 20
docker run --name node_23 -d --net mongo_net mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs2"; sleep 20
echo
