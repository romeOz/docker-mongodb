...
docker run --name base_1 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --noprealloc; sleep 20
docker exec -it base_1 mongo --eval 'db.createCollection("db_test");db.db_test.insert({name: "Tom"})'; sleep 5
docker exec -it base_1 mongo admin --eval 'db.createUser({user: "backuper", pwd: "pass", roles: [ "backup", "restore" ]})'; sleep 5
echo
echo "-- Backup"
...
docker run --name node_1 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --noprealloc --replSet "rs_test"; sleep 20
docker run --name node_2 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --noprealloc --replSet "rs_test"; sleep 20
docker run --name node_3 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --noprealloc --replSet "rs_test"; sleep 20
echo
echo "-- Configure replica set"
id_1=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' node_1)
id_2=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' node_2)
...
echo
echo "-- Create shards"
docker run --name node_1 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc; sleep 20
docker run --name node_2 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc; sleep 20

id_1=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' node_1)
id_2=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' node_2)

echo "-- Create a Config Server"
docker run --name cnf_1 -d mongo-3.2 --port 27017 --noprealloc --smallfiles --configsvr; sleep 20

id=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' cnf_1)
echo
echo "-- Create a Router (mongos)"
docker run --name mongos -d -e 'MONGO_MODE=mongos' mongo-3.2 --configdb ${id}:27017; sleep 20
echo
echo "-- Configure Router (mongos)"
docker exec -i mongos mongo <<EOF
sh.addShard("${id_1}:27017")
...
echo
echo "-- Create replica set #1"
docker run --name node_11 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs1"; sleep 20
docker run --name node_12 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs1"; sleep 20
docker run --name node_13 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs1"; sleep 20
echo
echo "-- Configure replica set #1"
id_11=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' node_11)
id_12=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' node_12)
...
echo
echo "-- Create replica set #2"
docker run --name node_21 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs2"; sleep 20
docker run --name node_22 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs2"; sleep 20
docker run --name node_23 -d mongo-3.2 --storageEngine wiredTiger --smallfiles --nojournal --noprealloc --replSet "rs2"; sleep 20
echo
echo "-- Configure replica set #2"
id_21=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' node_21)
id_22=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' node_22)
