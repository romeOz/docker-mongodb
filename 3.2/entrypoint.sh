...
RESTORE_OPTS=${RESTORE_OPTS:-"--host 127.0.0.1"}
...
start_server()
{
    sudo -Hu ${MONGO_USER} mongod --config ${MONGO_CONFIG} ${EXTRA_OPTS} &
    timeout=30
...
import_backup()
{
    ...
    start_server
    sudo -Hu ${MONGO_USER} mongorestore ${RESTORE_OPTS} --gzip --archive=${FILE}
}
...
if [[ ${MONGO_MODE} == backup ]]; then
    ...
    mongodump ${EXTRA_OPTS}  --archive=${MONGO_BACKUP_DIR}/backup.tar.gz --gzip
    rotate_backup
    exit 0
fi
...
if [[ -n ${MONGO_CHECK} ]]; then
    ...
    if [[ -z ${EXTRA_OPTS} ]]; then
        EXTRA_OPTS="--smallfiles --noprealloc"
    fi
